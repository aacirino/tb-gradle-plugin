...
class TBApplication extends TBProject {
	...
	void apply(Project project) {
		...
		configureTBApplicationTasks(project)
					...
					for (keyValuePair in deploymentMonitorBounceTasks) {
						...
						for (appName in appNameArray) {
							...
							task(taskName, dependsOn: copyTaskName) {
								...
								doLast {
									bounceTBApplication(logger, String.format(project.treasureboat.deploymentMonitorURLPattern, serverName), unixPath, appName)
								}
							}
	...
	def bounceTBApplication(logger, monitorURL, unixPath, appName) {
