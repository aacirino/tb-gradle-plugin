...
class TBProject implements Plugin<Project> {
	void apply(Project project) {
		...
		project.extensions.create('treasureboat', TBProjectPluginExtension)
		...
		configureTBProjectStructure(project)
		configureEclipseClasspath(project)
		configureTBProjectDependency(project)
		configureDependenciesTask(project)
		configureResourceTasks(project)
		configureJarTasks(project)
		installRepositories(project)
		addTBDependencies(project)
		addTestDependencies(project)
	}

	def configureTBProjectDependency(Project project) {
		project.with {
			def tbProjectURL = 'http://webobjects.mdimension.com/hudson/job/WOLips36Stable/lastSuccessfulBuild/artifact/woproject.jar'
			def tbProjectLibs = new File(project.projectDir, 'tbproject-libs')
			tbProjectLibs.mkdirs()
			
			def tbProjectLocalPath = new File(tbProjectLibs, 'tbproject.jar')
			if (!tbProjectLocalPath.exists()) {
				logger.warn 'Missing tbproject.jar, downloading to ' + tbProjectLocalPath.path + '...'
				
				def outputStream = tbProjectLocalPath.newOutputStream()
				outputStream << new URL(tbProjectURL).openStream()
				outputStream.close()
			} 
			
			configurations { tbproject }

			dependencies {
				// usually tbproject group:'org.objectstyle.TBProject.ant', name:'TBProject-ant-tasks', version:'2.0.16'
				tbproject files(tbProjectLocalPath)
			}
		}
	}

	def configureTBProjectStructure(Project project) {
		project.with {
			sourceSets {
				main {
	...
	def addTBDependencies(Project project) {
		project.dependencies {
			def tbVersion = project.treasureboat.tbVersion
...
class TBProjectPluginExtension {
